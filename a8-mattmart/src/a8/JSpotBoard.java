...
public class JSpotBoard extends JPanel implements SpotBoard {
	...
	public JSpotBoard copy(JSpot[][] thespots) {
		...
		for (int i=0; i<tWidth; i++) {
			for (int y=0; y<tHeight; y++) {
				this._spots[i][y].copy(thespots[i][y]);
			}
		}
	...
	public JSpot[][] returnspots() {
		//System.out.println("width: " + tWidth);
		JSpot[][] theSpots = new JSpot[tWidth][tHeight];
		for (int i=0;i<tWidth; i++) {
			for (int y=0;y<tHeight;y++) {
				JSpot p = this._spots[i][y];
				theSpots[i][y] = p;
			}
		}
		return theSpots;
